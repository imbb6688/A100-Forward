...
if not all(bool(manifest.get(k)) for k in ('full_market','complete','data_valid')):
 raise SystemExit('FAIL CLOSED: HiThink manifest is not explicitly complete/full-market/data-valid')
if manifest.get('latest_trade_date')!=str(today.date()):
 raise SystemExit('FAIL CLOSED: engine date differs from HiThink manifest')

status={
 'generated_at_utc':pd.Timestamp.utcnow().isoformat(),
 'latest_trade_date':str(today.date()),
 'hithink_latest_trade_date':manifest['latest_trade_date'],
 'market_gate':bool(gate_by_day[np.flatnonzero(dc==latest)[0]]) if np.any(dc==latest) else False,
 'candidate_count':int(mask.sum()),
 'top2':rows,
 'model':'A100 V7 FROZEN / ExtraTrees d4 leaf40',
 'execution':'Signals at close; simulated entry next trading day open; max 5 positions; 0.5% risk; 1.5ATR stop; 1.5R target; max hold 5d',
 'data_source':manifest.get('source','HiThink Financial-API full-market daily-k + adjustment events'),
 'validated_full_market':bool(manifest['full_market']),
 'data_valid':bool(manifest['data_valid']),
